...
for (int j=0; j< arity; j++)
  {nbtuples = nbtuples * tabdomaines[j].size();}
return nbtuples;
}
...
int NaryCSProblem::compute_conflict (Configuration* configuration, int var , int val)
{int value=0;
int aval=configuration->config[var];
...
{ 
  ...
  {
    int arity; int numvar; int defaultcost;
    file >> arity;
...
int narycsp(int argc,char** argv, int tuningmode)
{
  ...
  int nbvar,nbconst, domsize, lbound;
  int taille,nbessais;
  int graine1;
