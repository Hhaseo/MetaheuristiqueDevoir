...
void Stat_GWW::execution_report(int nessai, int lower_bound)
...
void sigaction()
  {Action.sa_handler=handler_stat;           // trombe_ajout : détournement du signal SIGUSR1
  sigaction(SIGUSR1, &Action, NULL);        // trombe_ajout : détournement du signal SIGUSR1
  }
...
void handler_stat (int sig) {
  ...
  *ofile  << "Signal " << sig << " recu !!" << endl;
  ...
  ecriture_stat_probleme();   // stats sur les derniers essais du problème courant

  if (Statistiques->current_pb > 1) {
    ecriture_statistiques_global (); // stats sur tous les problèmes
  }
...
/* les fonctions de lecture et de vérification de type et de valeur des données */
...
/* la liste des méthodes implantées */
...
liste_methodes.push_back("descente");
 liste_methodes.push_back("greedy");  // idw voisinage exploré (min=max) cf autotuning2.cc
...
void arguments_borneinf(char** argv, int& narg, int& borneinf)
...
{  graine1= argument2ul (argv[narg+1], " graine du générateur aléatoire de la méthode ");
  nbessais = argument2ul (argv[narg+2], " nombre d'essais ");
  method = argument2lp (argv[narg+3], "methode non implantee ", methodes_possibles);

  narg=narg+3;
  *ofile << " graine tirage aléatoire " << graine1 
	 << " nb essais " << nbessais << " méthode " << method << endl;
}
...
{
 walk_method = argument2lp (argv[narg+1], " methode marche non implantée " , liste_methodes);
...
BVNSAlgorithm* algo_vns(char** argv, int& narg, string& method , list<string>& liste_methodes)
{
 ...
 { IncompleteAlgorithm* algo = new IncompleteAlgorithm();
  ...
  if (method == "gww" || method =="gww-killworst"|| method == "gww-nothreshold" || method == "gww-adapt" || method == "gww-adaptkillworst"
    ...
    algo= algo_gww (argv,narg,method,taille,liste_methodes);	
	  ...
	  if (method=="vns") {
		  algo = algo_vns(argv,narg,method,liste_methodes);
		  taille = 1;
		  }
...
int valeur_max(Configuration** population, int taille)
{return population[0]->valuation;
}

/* la meilleure valeur de la population */
int valeur_min(Configuration** population, int taille)
{ return population[taille-1]->valuation; 
}

/* la valeur mediane de la population */
int valeur_mediane(Configuration** population, int taille)
...
/* le nom du fichier de sortie : pour les tests : resultatsxxx/concaténation des arguments */
...
void ecriture_fin_resolution(int meilleur)
...
void ecriture_debut_resolution(int pire, int meilleur,string& method)
...
void ecriture_stat_probleme() {	
  // role : écriture des stats pour le probleme courant avec les essais de 0 à current_try-1
  int current_problem;
  int nbessais;
  ...
  *ofile << "=========================================================================="<< endl;
  ...
  if (nbessais == 0) {
    ...
    *ofile << " " << Statistiques->trouve[current_problem] << " essais avec solutions sur " 
    ...
    double somme=0; double totalnbmoves=0; int meilleur=RAND_MAX; int pire=0; double ecart=0; double moyenne;
...
void ecriture_changement_seuil (int seuil, int delta, int meilleur, int pire,int mediane, int nbessaisvoisins, int nb_au_seuil)
{
  *ofile << " --- seuil " << seuil << " delta " << delta
	 ...
	 << " au seuil " << nb_au_seuil
    	 << " voisins testés " << nbessaisvoisins
         << endl;
}

void ecriture_nb_tues(int nb_tues)
{*ofile << " nombre particules redistribuées " << nb_tues << endl;}
...
void ecriture_meilleure_valeur (string & method, int valeur, int seuil, int nbmouv, int maxvoisins)
{
...
void ecriture_statistiques_global () {
  ...
  int pbsol;      // le nombre de problemes avec au moins un essai réussi
  int npb;        // le nombre de problemes etudiés
  int nb_succes;  // le nombre d'essais réussis sur tous les problemes
  ...
  *ofile<< " Nb succes " << nb_succes << " sur " 
	<< (npb-1) * Statistiques->max_tries +  Statistiques->current_try // nb d'essais achevés
	<< " essais " << endl;
...
{*ofile << " nb moyen d'essais par mouvement " << avgnhtries 
	<< "  ecart type " << sqrt ( avgsqnhtries - avgnhtries * avgnhtries) << endl;
 if (TRACEMODE==2)
...
void executer_essai
  ...
  {
    // graine du générateur aleatoire pour l'essai
    ...
    // pour les algos de type gww avec seuil , le seuil est initialisé au pire de la population, sinon à RAND_MAX
