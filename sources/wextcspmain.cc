...
/** le lecteur des fichiers au format ds de Simon de Givry
...
{WeightExtensionBinaryCSProblem*  p1 =new  WeightExtensionBinaryCSProblem (nbvar,nbconst,s);
 p1->cstweights=wei;
 p1->lconstraints=constraint1;
...
{
  WeightExtensionBinaryCSProblem* problem = weighted_csp (nbvar,nbconst,s,constraint1,constraint2,wei);
  wextensionbinary_setconnexions(nbvar,connexions,constraint1);
  return problem;
}
...
/* utilitaire : recherche la place d'un entier dans un vecteur d'entiers, renvoie -1 si pas trouvé*/
...
int*** constraint2,   vector<int> * tabdomaines, vector<int>& weight)
{
  ...
  while(tt != "%")
    ...
    while ((tt != "#") && (tt != "%"))
	 ...
	 nval1=value_number(atoi(tt2.c_str()),tabdomaines[n2]);}
      else
	{nval=value_number(atoi(tt2.c_str()),tabdomaines[n2]);
...
void dsdata_instance_read(ifstream& file, int& nbconst,vector<int>& variable1, vector<int>& variable2, vector<int>** constraint1,
			  vector<string> & constraint_names)
{
  ...
  while ((tt != "%") && (! file.eof()))
    ...
    constraint1[nv1][nv2].push_back(nc1); // on a nv1 < nv2
    nbconst++;
    file >> tt;
    }
}
...
int main (int argc, char** argv) {
  ...
  WeightExtensionBinaryCSProblem* problem ;          // pointeur sur le probleme
  ...
  int MAXCONST=1500;  // nb max de contraintes
  ...
  constraint1 =  wextcsp_constraintdatastructure(nbvar);
  ...
  while (! file.eof())
	...
	constraint1[i][j].clear();
    dsdata_instance_read(file,nbconst,variable1,variable2,constraint1,constraint_names);
    problem = weighted_csp_creation (nbvar,nbconst,maxdomsize,constraint1,constraint2, connexions, weight);
